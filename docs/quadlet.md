...
`findsenryu-app.container` は Podman healthcheck を有効にします。distroless image には shell が含まれないため、Dockerfile の `HEALTHCHECK CMD` 命令として直接埋め込んでいます。image に同梱した `/app/healthcheck` が container 内から `http://127.0.0.1:<findsenryu-server-port>/health` を確認します。
