...
`compose.yaml` は OCIR image、migrate、app、external secrets の基本形を示します。Quadlet は rootful system unit として運用し、Podman secret file mount と `UserNS=auto` を使います。
...
- `podman run --secret` / `--userns`: https://docs.podman.io/en/latest/markdown/podman-run.1.html
