...
GitHub Actions は Dockerfile からイメージをビルドして Oracle Cloud Infrastructure Registry (OCIR) に push します。Quadlet はそのイメージを参照します。deploy host での OCIR pull 認証は `docker-credential-ocir` と credential helper 名 `ocir` を使う前提です。
...
`.github/workflows/publish-ocir.yml` は `master` への push と手動実行で動作します。push する tag は `kix.ocir.io/axkvg5nxhc7t/senryu:latest` です。`senryu` repository は root compartment に置く前提です。push 後に OCI CLI で root compartment の AVAILABLE image を versioned / unversioned の両方から取得し、作成時刻の降順で直近 5 件だけを残して古い image を削除します。`latest` を上書き push した後の古い image は unversioned になるため、cleanup では `--is-versioned true` と `--is-versioned false` を両方指定して列挙します。
