...
named volume と `config.toml` bind mount は使いません。設定はすべて Podman secret として container 内の `/run/secrets/<target>` に mount します。Podman secret 名は配置環境の詳細であり、Quadlet の `Secret=<name>,target=<target>` でアプリケーション固定の mount 先へ対応付けます。PostgreSQL は外部にあるものへ接続する前提で、この repository では PostgreSQL container を用意しません。
...
journald が timestamp を保持するため、application log の本文には `time` を出力しません。`level` は journal 上で目視しやすいように本文へ残します。
...
`findsenryu-app.container` は Discord、PostgreSQL、log の secret を読みます。`findsenryu-migrate.container` は schema migration に必要な PostgreSQL と log の secret だけを読みます。

| Podman secret | container 内の mount 先 | 内容 | 空の場合 |
| --- | --- | --- | --- |
| `findsenryu-discord-token` | `/run/secrets/discord-token` | Discord Bot token | 不可 |
| `findsenryu-pghost` | `/run/secrets/pg-host` | PostgreSQL host | 不可 |
| `findsenryu-pgdatabase` | `/run/secrets/pg-database` | PostgreSQL database | 不可 |
| `findsenryu-pguser` | `/run/secrets/pg-user` | PostgreSQL user | 不可 |
| `findsenryu-pgpassword` | `/run/secrets/pg-password` | PostgreSQL password | 不可 |
| `findsenryu-pgsslmode` | `/run/secrets/pg-sslmode` | PostgreSQL sslmode。Neon では `verify-full` 推奨 | 不可 |
| `findsenryu-log-level` | `/run/secrets/log-level` | `debug` / `info` / `warn` / `error` | `info` |
