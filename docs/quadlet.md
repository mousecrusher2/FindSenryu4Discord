...
`findsenryu-app.container` は Podman healthcheck を有効にします。ベースとなる distroless image には shell が含まれないため、コンテナイメージ側には設定を埋め込まず、Quadlet ファイル (`findsenryu-app.container`) の `HealthCmd=["CMD", "/app/healthcheck"]` を使って shell を介さず直接バイナリを実行するよう設定しています。image に同梱した `/app/healthcheck` が container 内から `http://127.0.0.1:<findsenryu-server-port>/health` を確認します。

`HealthOnFailure=stop` を指定しているため、unhealthy になった container は Podman によって穏当に停止（SIGTERM送信後、一定時間内に停止しなければSIGKILL）され、`Restart=always` の systemd service として再起動されます。`findsenryu-server-enabled` を `false` にすると healthcheck 先が起動しないため、この Quadlet 構成では `true` のままにしてください。
