...
設定は Podman secret file で渡します。アプリケーションは `/run/secrets/<secret-name>` を直接読みます。データベースは PostgreSQL 専用です。SQLite は production runtime では使いません。
...
findsenryu-admin-log-channel-id
findsenryu-admin-report-channel-id
...
- **ヘルスチェック** — `/health`, `/ready`, `/stats` エンドポイントと container healthcheck
